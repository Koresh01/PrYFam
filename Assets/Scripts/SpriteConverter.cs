...
namespace PrYFam
{
    ...
    public static class SpriteConverter
    {
        ...
        public static string SpriteToString(Sprite sprite, string imageFormat = "png")
        {
            ...
            // Проверяем, доступна ли текстура для чтения, если нет — создаем копию
            if (!sprite.texture.isReadable)
            {
                // Создаем новую текстуру и копируем пиксели
                Texture2D readableTexture = new Texture2D((int)sprite.rect.width, (int)sprite.rect.height);
                readableTexture.SetPixels(sprite.texture.GetPixels((int)sprite.rect.x, (int)sprite.rect.y, (int)sprite.rect.width, (int)sprite.rect.height));
                readableTexture.Apply();

                // Теперь можем работать с этой текстурой
                texture = readableTexture;
            }
            else
            {
                // Если текстура доступна для чтения, просто копируем пиксели
